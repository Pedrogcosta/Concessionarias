...
string Executor::processarLinha(string linha) {
  ...
  if (nomeComando == "quit" ) {
    ...
    buf >> temppreco;
    buf >> tempano;
    preco = stoi(temppreco);
    ano = stoi(tempano);

    return sistema->add_car(marca,preco,chassi,ano,motor);
  }
